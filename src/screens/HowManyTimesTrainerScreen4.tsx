...
const HowManyTimesTrainerScreen4 = () => {
    ...
    const nextTask = () => {
        ...
        const newBase = Math.floor(Math.random() * 20) + 2; // 2–21
    ...
    const handleCheck = () => {
        ...
        const correctResult = type === 'więcej'
            ? baseNumber * multiplier
            : Math.floor(baseNumber / multiplier);
        ...
        <View style={{ flex: 1 }}>
            ...
            <KeyboardAwareScrollView contentContainerStyle={styles.container} enableOnAndroid extraScrollHeight={100} keyboardShouldPersistTaps="handled">
                <View style={{ position: 'absolute', top: 5, right: 5, alignItems: 'center', zIndex: 10 }}>
                    ...
                    <Text style={{ fontSize: 18, fontWeight: 'bold', color: '#007AFF', textAlign: 'center' }}>Pomóc</Text>
                    {showHint && (
                        <View style={{ marginTop: 5, padding: 10, backgroundColor: 'rgba(255,255,255,0.9)', borderRadius: 10, maxWidth: 200 }}>
                ...
                <Animated.View style={[styles.card, { backgroundColor: 'transparent' }]}>
                    ...
                    <Text style={styles.title}>Trener „ile razy więcej / ile razy mniej”</Text>

                    {!isGameFinished && (
                        <>
                            <Text style={styles.task}>
                                {type === 'więcej'
                                    ? `Znajdź liczbę ${baseNumber} × ${multiplier}`
                                    : `Znajdź liczbę ${baseNumber} ÷ ${multiplier}`}
                            </Text>
                            <TextInput style={[getValidationStyle(), styles.finalInput]} keyboardType="numeric" value={answer} onChangeText={setAnswer} placeholder="Odpowiedź" placeholderTextColor="#aaa" />
