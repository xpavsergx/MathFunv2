...
const NumberLineRenderer = ({ task }: { task: NumberLineTask }) => {
    ...
    const strokeColor = isDarkMode ? '#FFFFFF' : '#333';
    ...
    return (
        <View style={styles.svgContainer}>
            <Svg width="100%" height="100%" viewBox={`0 0 ${canvasWidth} ${canvasHeight}`}>
                <Line x1={5} y1={lineY} x2={canvasWidth - 15} y2={lineY} stroke={strokeColor} strokeWidth="3" />
                <Path d={`M ${canvasWidth - 25} ${lineY - 7} L ${canvasWidth - 10} ${lineY} L ${canvasWidth - 25} ${lineY + 7}`} stroke={strokeColor} strokeWidth="3" fill="none" />
                {task.start > 0 && <Path d={`M 5 ${lineY} L 20 ${lineY}`} stroke={strokeColor} strokeWidth="3" strokeDasharray="4, 4" />}
                {ticks}
            </Svg>
        </View>
    );
...
const FractionsNumberLineTrainerPL = () => {
    ...
    const theme = {
        ...
        lineColor: isDarkMode ? '#FFFFFF' : '#333',
    ...
    const getInputStyle = (isCorrect: boolean | null) => {
        const base = [styles.wholeInput, { backgroundColor: theme.inputBg, borderColor: theme.inputBorder, color: theme.inputText }];
        if (isCorrect === true) return [...base, { backgroundColor: theme.correctBg, borderColor: '#28a745', color: isDarkMode ? '#FFF' : '#28a745' }];
        if (isCorrect === false) return [...base, { backgroundColor: theme.errorBg, borderColor: '#dc3545', color: isDarkMode ? '#FFF' : '#dc3545' }];
        return base;
    };

    const getFractionInputStyle = (isCorrect: boolean | null) => {
        const base = [styles.fractionInput, { backgroundColor: theme.inputBg, borderColor: theme.inputBorder, color: theme.inputText }];
        if (isCorrect === true) return [...base, { backgroundColor: theme.correctBg, borderColor: '#28a745', color: isDarkMode ? '#FFF' : '#28a745' }];
        if (isCorrect === false) return [...base, { backgroundColor: theme.errorBg, borderColor: '#dc3545', color: isDarkMode ? '#FFF' : '#dc3545' }];
        return base;
    };
            ...
            <View style={{ flex: 1 }}>
                ...
                <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.keyboardContainer}>
                    ...
                    {showHint && taskData && !isKeyboardVisible && (
                        <View style={[styles.hintBox, { backgroundColor: theme.modalContent, borderColor: '#007AFF' }]}><Text style={styles.hintTitle}>Wskazówka:</Text><Text style={[styles.hintText, { color: theme.textMain }]}>{taskData.hint}</Text></View>
                    )}
                    ...
                    <Modal visible={showMilestone} transparent={true} animationType="slide">
                        <View style={styles.modalOverlay}>
                            <View style={[styles.milestoneCard, { backgroundColor: theme.modalContent }]}>
                                ...
                                <View style={[styles.statsRowMilestone, { backgroundColor: theme.statsRow, padding: 10, borderRadius: 10, width: '100%' }]}><Text style={[styles.statsTextMilestone, { color: theme.textMain }]}>Poprawne: {sessionCorrect} / 10</Text></View>
                    ...
                    <Modal visible={isFinished} transparent={true} animationType="fade">
                        <View style={styles.modalOverlay}>
                            <View style={[styles.milestoneCard, { backgroundColor: theme.modalContent }]}>
                                ...
                                <View style={[styles.statsRowMilestone, { backgroundColor: theme.statsRow, padding: 10, borderRadius: 10, width: '100%' }]}><Text style={[styles.statsTextMilestone, { color: theme.textMain }]}>Wynik: {stats.correct} / {TASKS_LIMIT}</Text></View>
                    ...
                    <ScrollView contentContainerStyle={styles.centerContent} keyboardShouldPersistTaps="handled">
                        <View style={styles.card}>
                            ...
                            <Text style={[styles.title, { color: theme.textMain }]}>Ułamki na Osi</Text>
                            <Text style={styles.taskLabel}>Jaką liczbę ukryto pod "?"</Text>
                            {taskData && <NumberLineRenderer task={taskData} />}
                            <View style={styles.answerSection}>
                                <View style={styles.wholeContainer}>
                                    <TextInput ref={wholeInputRef} style={getInputStyle(isWholeCorrect)} keyboardType="numeric" value={userWhole} onChangeText={(t) => {setUserWhole(t); setStatus('neutral');}} placeholder="0" placeholderTextColor={isDarkMode ? "#94A3B8" : "#ccc"} editable={!readyForNext} onSubmitEditing={() => numInputRef.current?.focus()} returnKeyType="next" />
                                    <Text style={[styles.labelSmall, { color: theme.textSub }]}>Całości</Text>
                                </View>
                                <View style={styles.fractionContainer}>
                                    <TextInput ref={numInputRef} style={getFractionInputStyle(isNumCorrect)} keyboardType="numeric" value={userNum} onChangeText={(v) => { setUserNum(v); setStatus('neutral'); if(v.length >= 1) denInputRef.current?.focus(); }} editable={!readyForNext} onSubmitEditing={() => denInputRef.current?.focus()} returnKeyType="next" placeholder="L" placeholderTextColor={isDarkMode ? "#94A3B8" : "#ccc"} />
                                    <View style={[styles.fractionLine, { backgroundColor: theme.lineColor }]} />
                                    <TextInput ref={denInputRef} style={getFractionInputStyle(isDenCorrect)} keyboardType="numeric" value={userDen} onChangeText={(t) => {setUserDen(t); setStatus('neutral');}} editable={!readyForNext} returnKeyType="done" placeholder="M" placeholderTextColor={isDarkMode ? "#94A3B8" : "#ccc"} />
                                </View>
                            </View>
                            ...
                            <Text style={[styles.counterTextSmall, { color: theme.textSub }]}>Zadanie: {stats.count > TASKS_LIMIT ? TASKS_LIMIT : stats.count} / {TASKS_LIMIT}</Text>
...
const styles = StyleSheet.create({
    ...
    title: { fontSize: 20, fontWeight: '800', marginBottom: 5, textTransform: 'uppercase' },
    ...
    wholeInput: { width: 60, height: 80, borderWidth: 2, borderRadius: 12, fontSize: 32, fontWeight: 'bold', textAlign: 'center' },
    ...
    fractionLine: { width: 70, height: 3, marginVertical: 4 },
    labelSmall: { fontSize: 10, marginTop: 4 },
    ...
    milestoneTitle: { fontSize: 22, fontWeight: 'bold', marginBottom: 10 },
