...
const FractionsAdditionTrainer = () => {
    ...
    const generateProblem = () => {
        ...
        do {
            ...
            let w1 = rnd(0, 4);
            let w2 = rnd(0, 4);
            if (w1 === 0 && w2 === 0 && Math.random() > 0.5) w1 = 1;
            ...
            let hintText = 'Dodaj całości do całości, a liczniki do liczników.';
            if (isTwoStep) {
                hintText += ' Jeśli licznik wyjdzie większy od mianownika, wyłącz całości. Jeśli można, skróć ułamek.';
            }
    ...
    const handleCheck = () => {
        ...
        if (allCorrect) {
            ...
            // --- SMART FEEDBACK (Podpowiedzi zamiast kasowania) ---

            // 1. Sprawdzamy czy wynik końcowy jest matematycznie równy, ale nieuproszczony
            if (task.isTwoStep && midCorrect && !finCorrect) {
                // Obliczamy wartość wpisaną przez usera w 2 kroku
                const userVal = uFinW + (uFinD !== 0 ? uFinN / uFinD : 0);
                const targetVal = task.finalWhole + (task.finalDen !== 0 ? task.finalNum / task.finalDen : 0);

                // Margines błędu dla float
                if (Math.abs(userVal - targetVal) < 0.0001) {
                    // Wartość poprawna, ale forma zapisu zła
                    if (uFinN > task.finalNum && uFinD > task.finalDen) {
                        setMsg('Skróć ułamek! ✂️'); // Trzeba skrócić
                        return; // NIE KASUJEMY PÓL
                    }
                    if (uFinN >= uFinD) {
                        setMsg('Wyłącz całości! 🍰'); // Trzeba wyłączyć całości
                        return; // NIE KASUJEMY PÓL
                    }
                }
            }
            ...
            if (attempts === 0) {
                ...
                // Kasujemy TYLKO to, co jest faktycznie źle
