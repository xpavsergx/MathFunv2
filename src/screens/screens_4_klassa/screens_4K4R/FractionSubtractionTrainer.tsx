...
// --- MODAL BRUDNOPISU Z OBSŁUGĄ TRYBU CIEMNEGO ---
...
const FractionSubtractionTrainer = () => {
    ...
    const generateProblem = () => {
        ...
        let uniqueKey = "";
        let attemptCount = 0;
        const denominators = [3, 4, 5, 6, 7, 8, 9, 10, 12, 15];
        ...
        do {
            ...
            const isTwoStep = (midNum !== finalNum) || (midWhole !== finalWhole) || (den !== finalDen);
            newTask = {
                q: `${w1} ${n1}/${den} - ${w2} ${n2}/${den}`,
                h: isTwoStep ? 'Odejmij, a następnie skróć ułamek lub wyłącz całości.' : 'Odejmij całości od całości i liczniki od liczników.',
                t1Whole: w1, t1Num: n1, t1Den: den,
                t2Whole: w2, t2Num: n2, t2Den: den,
                midWhole, midNum, midDen: den, hasMidWhole,
                finalWhole, finalNum, finalDen, hasFinalWhole: finalWhole > 0,
                isTwoStep
            };
    ...
    const handleCheck = () => {
        if ((task.hasMidWhole && !inpMidWhole) || !inpMidNum || !inpMidDen) { setMsg('Wypełnij pierwszy krok!'); return; }
        ...
        if (task.isTwoStep) {
            finCorrect = (task.hasFinalWhole ? uFinW === task.finalWhole : uFinW === 0) &&
                (task.finalNum > 0 ? (uFinN === task.finalNum && uFinD === task.finalDen) : (uFinN === 0));
        }
    ...
    const getValidationStyle = (currentVal: string, targetVal: number) => {
        const base = { backgroundColor: theme.inputBg, borderColor: theme.inputBorder, color: theme.inputText };
        if (status === 'neutral') return base;
        if (status === 'correct') return { ...base, backgroundColor: theme.correctBg, borderColor: '#28a745' };
        return (currentVal && parseInt(currentVal) === targetVal)
            ? { ...base, backgroundColor: theme.correctBg, borderColor: '#28a745' }
            : { ...base, backgroundColor: theme.errorBg, borderColor: '#dc3545' };
    };
                ...
                <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.keyboardContainer}>
                    ...
                    <Modal visible={isFinished} transparent={true} animationType="fade">
                        <View style={styles.modalOverlay}>
                            <View style={[styles.milestoneCard, { backgroundColor: theme.modalContent }]}>
                                ...
                                <View style={[styles.statsRowMilestone, { backgroundColor: theme.statsRow }]}><Text style={[styles.statsTextMilestone, { color: theme.textMain }]}>Wynik: {stats.correct} / {TASKS_LIMIT}</Text></View>
                                <View style={styles.milestoneButtons}>
                                    <TouchableOpacity style={[styles.mButton, {width: '48%'}]} onPress={handleRestart}><Text style={styles.mButtonText}>Od nowa</Text></TouchableOpacity>
                                    <TouchableOpacity style={[styles.mButton, {backgroundColor: '#dc3545', width: '48%'}]} onPress={() => navigation.goBack()}><Text style={styles.mButtonText}>Wyjdź</Text></TouchableOpacity>
                                </View>
                            </View>
                        </View>
                    </Modal>
                    ...
                    <ScrollView contentContainerStyle={styles.centerContent} keyboardShouldPersistTaps="handled">
                        <View style={styles.card}>
                            ...
                            <Text style={[styles.headerTitle, { color: '#007AFF' }]}>Odejmowanie ułamków</Text>
                            ...
                            <View style={styles.taskContent}>
                                <View style={styles.equationRow}>
                                    ...
                                    <View style={styles.mixedInputContainer}>
                                        {task.hasMidWhole && (
                                            <TextInput ref={midWholeRef} style={[styles.wholeInput, getValidationStyle(inpMidWhole, task.midWhole)]} keyboardType="numeric" value={inpMidWhole} onChangeText={t => {setInpMidWhole(t); setStatus('neutral');}} onSubmitEditing={() => midNumRef.current?.focus()} blurOnSubmit={false} editable={!ready} placeholder="?" placeholderTextColor={theme.inputPlaceholder} />
                                        )}
                                        <View style={styles.fractionInputColumn}>
                                            <TextInput ref={midNumRef} style={[styles.fractionInput, getValidationStyle(inpMidNum, task.midNum)]} keyboardType="numeric" value={inpMidNum} onChangeText={t => {setInpMidNum(t); setStatus('neutral');}} onSubmitEditing={() => midDenRef.current?.focus()} blurOnSubmit={false} editable={!ready} placeholder="?" placeholderTextColor={theme.inputPlaceholder} />
                                            <View style={[styles.fractionLineLarge, { backgroundColor: theme.lineColor }]} />
                                            <TextInput ref={midDenRef} style={[styles.fractionInput, getValidationStyle(inpMidDen, task.midDen)]} keyboardType="numeric" value={inpMidDen} onChangeText={t => {setInpMidDen(t); setStatus('neutral');}} onSubmitEditing={() => task.isTwoStep ? (task.hasFinalWhole ? finalWholeRef.current?.focus() : finalNumRef.current?.focus()) : handleCheck()} blurOnSubmit={!task.isTwoStep} editable={!ready} placeholder="?" placeholderTextColor={theme.inputPlaceholder} />
                                        </View>
                                    </View>
                                        ...
                                        <>
                                            ...
                                            <View style={styles.mixedInputContainer}>
                                                {(task.hasFinalWhole || task.finalNum === 0) && (
                                                    <TextInput ref={finalWholeRef} style={[styles.wholeInput, getValidationStyle(inpFinalWhole, task.finalWhole)]} keyboardType="numeric" value={inpFinalWhole} onChangeText={t => {setInpFinalWhole(t); setStatus('neutral');}} onSubmitEditing={() => task.finalNum > 0 && finalNumRef.current?.focus()} blurOnSubmit={false} editable={!ready} placeholder="?" placeholderTextColor={theme.inputPlaceholder} />
                                                )}
                                                {task.finalNum > 0 && (
                                                    <View style={styles.fractionInputColumn}>
                                                        <TextInput ref={finalNumRef} style={[styles.fractionInput, getValidationStyle(inpFinalNum, task.finalNum)]} keyboardType="numeric" value={inpFinalNum} onChangeText={t => {setInpFinalNum(t); setStatus('neutral');}} onSubmitEditing={() => finalDenRef.current?.focus()} blurOnSubmit={false} editable={!ready} placeholder="?" placeholderTextColor={theme.inputPlaceholder} />
                                                        <View style={[styles.fractionLineLarge, { backgroundColor: theme.lineColor }]} />
                                                        <TextInput ref={finalDenRef} style={[styles.fractionInput, getValidationStyle(inpFinalDen, task.finalDen)]} keyboardType="numeric" value={inpFinalDen} onChangeText={t => {setInpFinalDen(t); setStatus('neutral');}} onSubmitEditing={handleCheck} editable={!ready} placeholder="?" placeholderTextColor={theme.inputPlaceholder} />
                                                    </View>
                                                )}
                                            </View>
                                        </>
...
const styles = StyleSheet.create({
    ...
    headerTitle: { fontSize: 22, fontWeight: '800', marginBottom: 20, textTransform: 'uppercase' },
    ...
    mButton: { paddingVertical: 12, borderRadius: 12, alignItems: 'center', backgroundColor: '#28a745' },
    mButtonText: { color: '#fff', fontWeight: 'bold', fontSize: 16 },
