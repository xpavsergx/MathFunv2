...
const MassUnitsTrainer = () => {
    ...
    const generateProblem = () => {
        ...
        while (true) {
            ...
            let q = 'Przelicz jednostki masy:';
            let sub = ''; let ans1 = ''; let ans2 = ''; let hint = ''; let opts: string[] = [];

            if (type < 0.4) {
                const pairs = [{f:'kg', t:'dag', m:100}, {f:'dag', t:'g', m:10}, {f:'kg', t:'g', m:1000}, {f:'t', t:'kg', m:1000}];
                const p = pairs[Math.floor(Math.random() * pairs.length)];
                const val = Math.floor(Math.random() * 80) + 11;
                const correct = val * p.m;
                ans1 = correct.toString();
                ...
                s.add(ans1);
                while(s.size < 4) {
                    const fakeOptions = [`${val}`, `${correct * 10}`, `${val}0${val % 10}`, `${correct / 10}`, `${val * 10}`];
                    s.add(fakeOptions[Math.floor(Math.random() * fakeOptions.length)]);
                }
                opts = Array.from(s).sort(() => Math.random() - 0.5);
                hint = `1 ${p.f} = ${p.m} ${p.t}. Pomnóż ${val} przez ${p.m}.`;
                id = `opt-${val}-${p.f}`;
            } else if (type < 0.75) {
                const pairs = [{f:'g', t:'dag', d:10}, {f:'dag', t:'kg', d:100}, {f:'kg', t:'t', d:1000}, {f:'g', t:'kg', d:1000}];
                const p = pairs[Math.floor(Math.random() * pairs.length)];
                const val = (Math.floor(Math.random() * 50) + 1) * p.d;
                sub = `${val} ${p.f} = ... ${p.t}`;
                ans1 = (val / p.d).toString();
                hint = `1 ${p.t} = ${p.d} ${p.f}. Podziel ${val} przez ${p.d}.`;
                id = `div-${val}-${p.f}`;
            } else {
                const r = Math.random();
                if (r < 0.5) {
                    const val = Math.floor(Math.random() * 800) + 110;
                    sub = `${val} dag = ... kg ... dag`;
                    ...
                    const val = Math.floor(Math.random() * 5000) + 1100;
                    sub = `${val} g = ... kg ... g`;
                    ans1 = Math.floor(val / 1000).toString();
                    ans2 = (val % 1000).toString();
                    hint = "1 kg = 1000 g.";
                    id = `rem-g-${val}`;
                }
            }
    ...
    const handleCheck = (selectedVal?: string) => {
        ...
        if (check1 && check2) {
            setIsInput1Correct(true); setIsInput2Correct(true);
            ...
            setIsInput1Correct(check1); setIsInput2Correct(check2);
            Animated.sequence([
                                        ...
                                        return (
                                            <TouchableOpacity key={index}
                                                              style={[
                                                                  ...
                                                                  showGreen && styles.testCorrect,
                                                                  showRed && styles.testError
                                                              ]}
                                                              onPress={() => !readyForNext && handleCheck(opt)}>
                                                <Text style={[styles.testOptionText, (showGreen || showRed) ? {color: '#fff'} : {color: isDarkMode ? '#FFF' : '#007AFF'}]}>{opt}</Text>
                                            </TouchableOpacity>
                                        );
                                    })}
                                </View>
                                ...
                                <View style={{ flexDirection: 'row', justifyContent: 'center' }}>
                                    <TextInput
                                        style={[
                                            ...
                                            isInput1Correct === true && { backgroundColor: theme.correctBg, borderColor: '#28a745' },
                                            isInput1Correct === false && { backgroundColor: theme.errorBg, borderColor: '#dc3545' },
                                            correctAnswer2 !== '' && {width:125}
                                        ]}
                                        keyboardType="numeric" value={userInput} onChangeText={(t) => {setUserInput(t); if(isInput1Correct === false) setIsInput1Correct(null);}}
                                        editable={!readyForNext} placeholder="?" placeholderTextColor={theme.inputPlaceholder}
                                    />
                                    {correctAnswer2 !== '' && (
                                        <TextInput
                                            style={[
                                                styles.finalInput,
                                                {marginLeft:10, width:125, backgroundColor: theme.inputBg, borderColor: theme.inputBorder, color: theme.inputText},
                                                isInput2Correct === true && { backgroundColor: theme.correctBg, borderColor: '#28a745' },
                                                isInput2Correct === false && { backgroundColor: theme.errorBg, borderColor: '#dc3545' }
                                            ]}
                                            keyboardType="numeric" value={userInput2} onChangeText={(t) => {setUserInput2(t); if(isInput2Correct === false) setIsInput2Correct(null);}}
                                            editable={!readyForNext} placeholder="?" placeholderTextColor={theme.inputPlaceholder}
                                        />
                                    )}
                                </View>
...
export default MassUnitsTrainer;