...
const MassUnitsTrainer = () => {
    ...
    const generateProblem = () => {
        ...
        while (true) {
            ...
            if (type < 0.4) { // ТЕСТ (Исправленная логика вариантов)
                ...
                // Создаем наборы уникальных ответов
                ...
                while(s.size < 4) {
                    ...
                    // Если всё еще не хватает, добавляем случайное близкое число
                    if(s.size < 4) s.add((correct + (Math.random() > 0.5 ? 10 : -10)).toString());
                }
                opts = Array.from(s).sort(() => Math.random() - 0.5);

                hint = `1 ${p.f} = ${p.m} ${p.t}. Pomnóż ${val} przez ${p.m}.`;
                id = `opt-${val}-${p.f}`;
            } else if (type < 0.75) { // ДЕЛЕНИЕ
