...
const ComparingNumbersTrainer = () => {
    ...
    // Blokada interakcji podczas animacji błędu
    ...
    const generateProblem = (type: string) => {
        ...
        if (type === 'symbols') {
            ...
            nums = [...new Set(nums)];
            while(nums.length < 3) {
                nums.push(nums[0] + nums.length + 1);
            }
            ...
            if (puzzleStyle < 0.5) {
                ...
                setMainDisplay(
                    <Text style={styles.rangeText}>
                        {formatNumber(start)} {' < x < '} {formatNumber(end)}
                    </Text>
                );
    ...
    const handleCheck = () => {
        ...
        if (isProcessing) return; // Zapobiega wielokrotnemu klikaniu podczas animacji błędu
        ...
        if (isOk) {
            ...
            if (firstAttempt) {
                ...
                // Jeśli to zadanie z przyciskami, opóźniamy czyszczenie
                if (options.length > 0) {
                    setIsProcessing(true); // Blokujemy klikanie
                    setTimeout(() => {
                        setUserInput(''); // Czyści zaznaczenie
                        setIsCorrect(null); // Resetuje kolor
                        setIsProcessing(false); // Odblokowuje
                    }, 1000); // 1 sekunda opóźnienia
                } else {
                    // Dla wpisywania ręcznego czyścimy od razu lub też opóźniamy - zróbmy opóźnienie dla spójności
                ...
                if (correctStr.startsWith('RANGE:')) {
                    const parts = correctStr.split(':');
                    setMessage(`Przykładowa poprawna: ${formatNumber(parts[1])}`);
                } else {
    ...
    const getFieldStyle = () => {
        if (isCorrect === true) return styles.correctFinal;
        if (isCorrect === false) return styles.errorFinal;
                                ...
                                <View key={`opts-${taskCount}`} style={styles.optionsContainer}>
                                    {options.map((opt, idx) => {
                                        ...
                                        return (
                                            <TouchableOpacity
                                                key={idx}
                                                onPress={() => !readyForNext && !isProcessing && setUserInput(opt)}
                                                style={[
                                                    styles.optionButton,
                                                    isSymbol ? styles.symbolButton : styles.numberButton,
                                                    isSelected && styles.optionButtonSelected,
                                                    // Logika kolorowania przycisków
                                                    (readyForNext || (isSelected && isCorrect === true)) && opt === correctAnswer.toString() && styles.optionButtonCorrect,
