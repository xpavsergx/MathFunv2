...
const EXERCISE_ID = "WrittenAdditionTrainer";
...
const WrittenAdditionTrainer = () => {
    ...
    const theme = {
        ...
        // Клетки для цифр
        cellBg: isDarkMode ? '#334155' : '#fff',
        cellBorder: isDarkMode ? '#475569' : '#ccc',

        // Перенос (carry)
        carryBg: isDarkMode ? '#1E293B' : '#f9f9f9',
    ...
    const generateProblem = () => {
        ...
        const n2 = Math.floor(Math.pow(10, l2-1) + Math.random() * (Math.pow(10, l2) - Math.pow(10, l2-1)));
        const sum = n1 + n2;
        const sumStr = sum.toString();
        setNum1(n1.toString());
        setNum2(n2.toString());
        setFullResult(sum);
        setUserDigits(new Array(sumStr.length).fill(''));
        setCarries(new Array(sumStr.length).fill(''));
        setTimeout(() => inputRefs.current[sumStr.length - 1]?.focus(), 500);
    };
    ...
    const handleCheck = () => {
        ...
        if (userResStr === "" || userDigits.some(d => d === "")) {
    ...
    const maxLen = userDigits.length;
    const n1Padded = num1.padStart(maxLen, ' ');
    const n2Padded = num2.padStart(maxLen, ' ');
            ...
            <View style={styles.container}>
                ...
                <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.keyboardContainer}>
                    ...
                    {showHint && !isKeyboardVisible && (
                        <View style={[styles.hintBox, { backgroundColor: isDarkMode ? '#1E293B' : '#fff' }]}>
                            <Text style={styles.hintTitle}>Podpowiedź:</Text>
                            <Text style={[styles.hintText, { color: theme.textMain }]}>Zacznij od prawej strony. Małe okienka na sameй górze służą do zapisywania cyfr "w pamięci".</Text>
                        </View>
                    )}
                    ...
                    <Modal visible={showMilestone} transparent={true} animationType="slide">
                        <View style={styles.modalOverlay}>
                            <View style={[styles.milestoneCard, { backgroundColor: isDarkMode ? '#1E293B' : '#fff' }]}>
                                <Text style={[styles.milestoneTitle, { color: theme.textMain }]}>Podsumowanie serii 📊</Text>
                                <View style={[styles.statsRow, { backgroundColor: isDarkMode ? '#0F172A' : '#f8f9fa' }]}>
                    ...
                    <ScrollView contentContainerStyle={styles.centerContent} keyboardShouldPersistTaps="handled">
                        <View style={styles.card}>
                            ...
                            <Text style={[styles.questionMain, { color: theme.textMain }]}>Dodawanie pisemne</Text>

                            <View style={styles.columnContainer}>
                                {/* РЯД ДЛЯ ПЕРЕНОСА (CARRY) */}
                                <View style={[styles.row, { marginBottom: 5 }]}>
                                    <View style={styles.opSpace} />
                                    {carries.map((c, i) => (
                                        <View key={`carry-box-${i}`} style={[styles.carryCell, { backgroundColor: theme.carryBg, borderColor: theme.cellBorder }]}>
                                            <TextInput
                                ...
                                <View style={styles.row}>
                                    <View style={styles.opSpace} />
                                    {userDigits.map((d, i) => {
                                        let cellStyle = [styles.inputCell, { backgroundColor: theme.cellBg, borderColor: theme.cellBorder }];
                                        ...
                                        return (
                                            <View key={`ans-box-${i}`} style={cellStyle}>
                                                <TextInput
...
export default WrittenAdditionTrainer;