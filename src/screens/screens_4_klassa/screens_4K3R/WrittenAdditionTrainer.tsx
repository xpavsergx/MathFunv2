...
const TASKS_LIMIT = 50; // ИЗМЕНЕНО: теперь ограничение 50 примеров
...
const WrittenAdditionTrainer = () => {
    ...
    const generateProblem = () => {
        ...
        setTimeout(() => {
            if (inputRefs.current[sumStr.length - 1]) {
                inputRefs.current[sumStr.length - 1]?.focus();
            }
        }, 500);
    };
    ...
    const handleCheck = () => {
        ...
        if (userRes === fullResult) {
            ...
            if (firstAttempt) {
                setMessage('Błąd. Spróbuj еще raz.');
                ...
                <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.keyboardContainer}>
                    ...
                    {showHint && !isKeyboardVisible && (
                        <View style={styles.hintBox}>
                            ...
                            <Text style={styles.hintText}>Zacznij od prawej strony. Małe okienka na samej górze służą do zapisywania cyfr "w pamięci".</Text>
                        </View>
                    )}
                    ...
                    <ScrollView contentContainerStyle={styles.centerContent} keyboardShouldPersistTaps="handled">
                        <View style={styles.card}>
                            ...
                            <Text style={styles.questionMain}>Dodawanie pisemne</Text>

                            <View style={styles.columnContainer}>
                                <View style={[styles.row, { marginBottom: 5 }]}>
                                    <View style={styles.opSpace} />
                                    {carries.map((c, i) => (
                                        <View key={`carry-box-${i}`} style={styles.carryCell}>
                                            <TextInput
