...
import {
    ...
    ScrollView, InteractionManager, Platform, useColorScheme
} from 'react-native';
...
const DecimalComparisonTrainer = () => {
    ...
    const theme = {
        ...
        btnBg: isDarkMode ? '#334155' : '#fff',
        btnBorder: isDarkMode ? '#475569' : '#007AFF',
        signBoxNeutral: isDarkMode ? '#334155' : '#f9f9f9',
        signBoxBorder: isDarkMode ? '#475569' : '#ccc',
    };
    ...
    const generateProblem = () => {
        ...
        let n1Str = ""; let n2Str = ""; let sign = ""; let uniqueKey = ""; let attemptCount = 0; let hintText = "";

        do {
            const type = rnd(1, 100);
            if (type <= 30) {
                const whole = rnd(0, 15); const digitA = rnd(2, 9); const digitB = digitA - 1;
                n1Str = `${whole},${digitA}`; const tail = rnd(10, 99); n2Str = `${whole},${digitB}${tail}`;
                ...
                const whole = rnd(0, 50); const digit = rnd(1, 9);
                if (Math.random() > 0.5) { n1Str = `${whole},0${digit}`; n2Str = `${whole},${digit}`; }
                else { n1Str = `${whole},00${rnd(1,9)}`; n2Str = `${whole},0${rnd(1,9)}`; }
                hintText = "Zwróć uwagę, gdzie stoi zero po przecinku.";
                if (Math.random() > 0.5) { const temp = n1Str; n1Str = n2Str; n2Str = temp; }
            } else if (type <= 75) {
                const whole = rnd(0, 20); const base = rnd(10, 99); const end1 = rnd(0, 9); let end2 = rnd(0, 9);
                while(end2 === end1) end2 = rnd(0, 9);
                n1Str = `${whole},${base}${end1}`; n2Str = `${whole},${base}${end2}`;
                hintText = "Liczby różnią się dopiero na trzecim miejscu po przecinku.";
            } else if (type <= 90) {
                const whole = rnd(0, 99); const dec = rnd(1, 99);
                n1Str = `${whole},${dec}`; n2Str = `${whole},${dec}00`;
                hintText = "Czy dopisanie zer na końcu po przecinku zmienia wartość?";
                if (Math.random() > 0.5) { const temp = n1Str; n1Str = n2Str; n2Str = temp; }
            } else {
                const w1 = rnd(100, 999); const w2 = w1 - 1;
                n1Str = `${w1},${rnd(0,5)}`; n2Str = `${w2},${rnd(800,999)}`;
            ...
            if (val1 > val2) sign = '>'; else if (val1 < val2) sign = '<'; else sign = '=';
            uniqueKey = `${n1Str}|${n2Str}`; attemptCount++;
        } while (historyRef.current.has(uniqueKey) && attemptCount < 20);

        historyRef.current.add(uniqueKey);
        setTask({ q: 'Wstaw znak <, > lub =', h: hintText, val1Str: n1Str, val2Str: n2Str, correctSign: sign });
    };

    const handleSelectSign = (sign: string) => {
        if (ready) return;
        setCurrentSign(sign); setBoxStatus('neutral'); setMsg('');
    };

    const handleCheck = () => {
        if (currentSign === '?') { setMsg('Wybierz znak z dołu! ⚠️'); return; }

        if (currentSign === task.correctSign) {
            setBoxStatus('correct'); setMsg('Doskonale! ✅');
            ...
            setBoxStatus('wrong');
            if (task.correctSign === '=' && currentSign !== '=') setMsg('Te liczby są równe (zera nie zmieniają wartości).');
            else if (task.val1Str.length !== task.val2Str.length && Math.random() > 0.5) setMsg('Nie sugeruj się długością liczby! 📏');
            else setMsg('Błąd... Spróbuj poprawić! ❌');
            ...
            if (attempts === 0) {
                ...
                setMsg(`Poprawny znak to: ${task.correctSign}`); setReady(true); setStats(s => ({ ...s, wrong: s.wrong + 1 }));
                Animated.timing(bgAnim, { toValue: -1, duration: 500, useNativeDriver: false }).start();
                const currentUser = auth().currentUser;
                if (currentUser) {
                    firestore().collection('users').doc(currentUser.uid).collection('exerciseStats').doc(EXERCISE_ID)
    ...
    const handleContinueMilestone = () => { setShowMilestone(false); setSessionCorrect(0); setStats(s => ({ ...s, count: s.count + 1 })); generateProblem(); };

    const handleRestart = () => { setIsFinished(false); setShowMilestone(false); setStats({ correct: 0, wrong: 0, count: 1 }); setSessionCorrect(0); historyRef.current.clear(); generateProblem(); };
    ...
    return (
        <View style={styles.container}>
            ...
            <Modal visible={showMilestone} transparent={true} animationType="slide">
                <View style={styles.modalOverlay}>
                    <View style={[styles.milestoneCard, { backgroundColor: theme.modalContent }]}>
                        ...
                        <Text style={[styles.suggestionTextMilestone, { color: theme.textSub }]}>{sessionCorrect >= 8 ? "Jesteś ekspertem od ułamków!" : "Uważaj na zera po przecinku. Trenuj dalej!"}</Text>
            ...
            <Modal visible={isFinished} transparent={true} animationType="fade">
                <View style={styles.modalOverlay}>
                    <View style={[styles.milestoneCard, { backgroundColor: theme.modalContent }]}>
                        ...
                        <View style={[styles.statsRowMilestone, { backgroundColor: theme.statsRow }]}>
                            <Text style={[styles.statsTextMilestone, { color: theme.textMain }]}>Twój wynik końcowy:</Text>
                            <Text style={[styles.statsTextMilestone, { fontSize: 24, color: '#28a745', marginTop: 5 }]}>{stats.correct} / {TASKS_LIMIT}</Text>
                        </View>
                        <View style={styles.milestoneButtons}>
                            <TouchableOpacity style={[styles.mButton, { backgroundColor: '#28a745' }]} onPress={handleRestart}><Text style={styles.mButtonText}>Zagraj jeszcze raz</Text></TouchableOpacity>
            ...
            <DrawingModal visible={showScratchpad} onClose={() => setShowScratchpad(false)} problemText={task.val1Str + " ? " + task.val2Str} />

            <ScrollView contentContainerStyle={styles.centerContent}>
                <View style={styles.card}>
                    <View style={[styles.overlayBackground, { backgroundColor: theme.cardOverlay }]} />
                    <Text style={[styles.headerTitle, { color: theme.textMain }]}>Porównywanie ułamków</Text>
                    <Text style={[styles.questionText, { color: theme.textSub }]}>{task.q}</Text>

                    <View style={styles.comparisonContainer}>
                        <View style={styles.sideContainer}>
                            <View style={styles.numberDisplay}>
                                <Text style={[styles.decimalText, { color: theme.textMain }, task.val1Str.length > 5 && styles.decimalTextSmall]}>{task.val1Str}</Text>
                            </View>
                        </View>
                            ...
                            <View style={[
                                ...
                                boxStatus === 'correct' ? styles.signBoxCorrect :
                                    boxStatus === 'wrong' ? styles.signBoxWrong :
                                        { backgroundColor: theme.signBoxNeutral, borderColor: theme.signBoxBorder }
                            ]}>
                                <Text style={[
                                    styles.signText,
                                    boxStatus === 'neutral' && currentSign === '?' ? {color: isDarkMode ? '#475569' : '#ccc'} :
                        ...
                        <View style={styles.sideContainer}>
                            <View style={styles.numberDisplay}>
                                <Text style={[styles.decimalText, { color: theme.textMain }, task.val2Str.length > 5 && styles.decimalTextSmall]}>{task.val2Str}</Text>
                            </View>
                        </View>
                    </View>

                    <View style={styles.answersRow}>
                        <TouchableOpacity style={[styles.answerBtn, { backgroundColor: theme.btnBg, borderColor: theme.btnBorder }, currentSign === '<' && styles.answerBtnSelected]} onPress={() => handleSelectSign('<')}><Text style={[styles.answerBtnText, { color: theme.topBtnText }]}>&lt;</Text></TouchableOpacity>
                        <TouchableOpacity style={[styles.answerBtn, { backgroundColor: theme.btnBg, borderColor: theme.btnBorder }, currentSign === '=' && styles.answerBtnSelected]} onPress={() => handleSelectSign('=')}><Text style={[styles.answerBtnText, { color: theme.topBtnText }]}>=</Text></TouchableOpacity>
                        <TouchableOpacity style={[styles.answerBtn, { backgroundColor: theme.btnBg, borderColor: theme.btnBorder }, currentSign === '>' && styles.answerBtnSelected]} onPress={() => handleSelectSign('>')}><Text style={[styles.answerBtnText, { color: theme.topBtnText }]}>&gt;</Text></TouchableOpacity>
                    </View>

                    <TouchableOpacity style={styles.mainBtn} onPress={ready ? nextTask : handleCheck}><Text style={styles.mainBtnText}>{ready ? 'Następne' : 'Sprawdź'}</Text></TouchableOpacity>
...
const styles = StyleSheet.create({
    ...
    decimalText: { fontSize: 34, fontWeight: 'bold' },
    decimalTextSmall: { fontSize: 26 },
    ...
    answerBtn: { borderWidth: 2, borderRadius: 15, width: 70, height: 70, justifyContent: 'center', alignItems: 'center', elevation: 3 },
    answerBtnSelected: { backgroundColor: '#eef6ff', borderColor: '#005bb5' },
    answerBtnText: { fontSize: 36, fontWeight: 'bold' },
    ...
    suggestionTextMilestone: { fontSize: 15, textAlign: 'center', marginVertical: 20, lineHeight: 22 },
