    ...
    const generateProblem = () => {
        ...
        if (newMode === 'mental') {
            ...
            if (subType < 0.4) {
                ...
                const decimal1 = Math.floor(Math.random() * 8) + 1;
                const maxDecimal2 = 9 - decimal1;
                const decimal2 = Math.floor(Math.random() * maxDecimal2) + 1;
                n1 = parseFloat(`${whole}.${decimal1}`);
                n2 = parseFloat(`0.${decimal2}`);
            } else if (subType < 0.7) {
                const whole = Math.floor(Math.random() * 4);
                const decimal1 = Math.floor(Math.random() * 9) + 1;
                const decimal2 = 10 - decimal1;
                n1 = parseFloat(`${whole}.${decimal1}`);
                n2 = parseFloat(`0.${decimal2}`);
            } else {
                const whole1 = Math.floor(Math.random() * 3);
                const whole2 = Math.floor(Math.random() * 3);
                const decimal1 = Math.floor(Math.random() * 5) + 5;
                const decimal2 = Math.floor(Math.random() * 5) + 5;
                n1 = parseFloat(`${whole1}.${decimal1}`);
                n2 = parseFloat(`${whole2}.${decimal2}`);
            }
            setNum1(n1); setNum2(n2);

        } else {
            // Logika słupkowa - zawsze 2 miejsca po przecinku
            const n1Int = Math.floor(Math.random() * 50);
            const n2Int = Math.floor(Math.random() * 50);
            const n1Dec = Math.floor(Math.random() * 99);
            const n2Dec = Math.floor(Math.random() * 99);

            const rawVal1 = n1Int + (n1Dec / 100);
            const rawVal2 = n2Int + (n2Dec / 100);
    ...
    const handleCheck = () => {
        ...
        // --- VERTICAL MODE ---
            ...
            for (let i = 0; i < userDigits.length; i++) {
                ...
                // Jeśli oczekujemy cyfry (nie spacji), a pole jest puste i nie jest zablokowane -> BŁĄD BRAKU DANYCH
            ...
            // Jeśli są puste pola -> przerywamy, nie liczymy próby!
            if (hasEmptyFields) {
            ...
            if (allGood) {
                ...
                if (attempts === 0) {
                    ...
                    for (let i = 0; i < userDigits.length; i++) {
                        ...
                        if (inputVal === expectedVal) {
                            newLocked[i] = true; // Zablokuj
                            newErrors[i] = false;
                        } else {
                            newErrors[i] = true; // Błąd
                            newUserDigits[i] = ''; // Wyczyść
                        }
                    }
                    ...
                    {showHint && !isKeyboardVisible && (
                        <View style={styles.hintBox}>
                            ...
                            <Text style={styles.hintText}>
                                {mode === 'mental'
                                    ? "Masz dwie szanse. Pamiętaj o przecinku!"
                                    : "Uzupełnij wszystkie pola przed sprawdzeniem. Puste miejsca to zera."}
                            </Text>
                        </View>
                    )}
                                ...
                                {mode === 'mental' ? (
                                    <View style={styles.mentalContainer}>
                                        ...
                                        <TextInput
                                            ...
                                            onChangeText={(t) => {
                                                setMentalInput(t);
                                                if(attempts === 1) setStatus('neutral');
                                            }}
                                            editable={!readyForNext}
                                        />
                                    </View>
