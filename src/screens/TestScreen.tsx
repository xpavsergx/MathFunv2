...
interface Question {
    ...
    difficulty: 'łatwe' | 'średnie' | 'trudne'; // Рівень складності
    questionText: string;
    options: string[];
...
// Тип для даних підтеми, що містить масив питань
type SubTopicData = {
    theoryTitle?: string;
    theoryContent?: any[]; // Може бути більш конкретним, якщо відома структура теорії
    questions?: Question[]; // Питання тепер у цьому полі
};

// Тип для questionsDb.json, що відображає нову структуру
type QuestionsDatabase = {
    [grade: string]: { // Клас (наприклад, "4")
        [topic: string]: { // Розділ (наприклад, "LICZBY I DZIAŁANIA")
            [subTopic: string]: SubTopicData; // Підтема тепер об'єкт, що містить питання та теорію
        };
    };
};
...
function TestScreen({ route, navigation }: TestScreenProps) {
    ...
    useEffect(() => {
        ...
        if (currentTestType === 'mainTopic' && grade && topic) {
            ...
            if (subTopicsMap) {
                Object.keys(subTopicsMap).forEach(subTopicKey => {
                    // Питання тепер знаходяться у полі 'questions' об'єкта підтеми
                ...
                if (loadedQuestions.length > 20) { // Обмеження кількості питань для тесту за розділ
                    loadedQuestions = loadedQuestions.slice(0, 20);
                }
            }
        ...
        // TODO: Додати логіку для 'gradeRandom', 'gradeAssessment'
    ...
    }, [mode, questions.length, navigation, score, currentQuestionIndex, route.params]);
    ...
    const handleAnswerSelect = (index: number) => {
        if (!isAnswerSubmitted || (mode === 'learn' && !showFeedback) ) { // Дозволяємо змінювати відповідь до показу фідбеку в режимі навчання
    ...
    if (!currentQuestion) {
        // Це може статися, якщо currentQuestionIndex вийде за межі questions, хоча handleNextQuestion має це запобігти
    ...
    return (
        <ScrollView style={styles.scrollView} contentContainerStyle={styles.container}>
            ...
            {currentQuestion.type === 'practice' && (
                <View style={styles.optionsContainer}>
                    {currentQuestion.options.map((option: string, index: number) => {
                        ...
                        if (isAnswerSubmitted && showFeedback && mode === 'learn') { // Фідбек показується тільки після відповіді і якщо він має бути показаний
                            if (isCorrect) {
                                buttonStyle = [styles.optionButton, styles.correctOption];
                                textStyle = [styles.optionText, styles.correctOptionText];
                            } else if (isSelected) { // Неправильно вибраний
                                buttonStyle = [styles.optionButton, styles.incorrectOption];
                            }
                        } else if (isSelected) { // Просто вибраний, ще не підтверджено або режим оцінки
                        ...
                        return (
                            <TouchableOpacity
                                ...
                                disabled={isAnswerSubmitted && mode === 'assess'} // Блокуємо зміну в режимі оцінки після відповіді
                            >
            ...
            {(!isAnswerSubmitted || (isAnswerSubmitted && !showFeedback && mode === 'assess' && currentQuestion.type === 'practice' )) && ( // Кнопка відповіді/далі для теорії
